...
2. PDFを取得
3. PyMuPDFで罫線付き表を検出
4. 表構成を判定して避難所名と避難者数を取得
5. PDFの合計避難者数と各行の合計を照合

文字列の空白位置や住所列の有無には依存せず、表の列構造と設備記号の位置を使用します。
...
日本時間の毎日8時10分、8時30分、18時30分の3回確認します。朝の2回は宇城市など8時頃の更新を取り込み、18時30分は八代市の18時時点資料を当日中に取り込むための確認です。0時05分の日次避難所収集との直接連動は行いません。手動実行も可能です。

GitHub ActionsのcronはUTCで指定するため、ワークフローでは次を設定しています。

```yaml
schedule:
  - cron: "10 23 * * *"  # 08:10 JST
  - cron: "30 23 * * *"  # 08:30 JST
  - cron: "30 9 * * *"   # 18:30 JST
```

同じ排他グループを使用し、先行処理が長引いた場合も後続確認をキャンセルせず順番に実行します。

8時10分時点で情報源が更新されていなければ新しい観測は追加せず、8時30分に再確認します。18時30分にも再確認し、自治体公表時刻または正規化ハッシュが変化していれば追加します。3回とも情報源に実質的な変更がなければCSVやコミットは追加しません。処理失敗時はHTML、PDF、実行ログをGitHub Actions artifactへ保存します。
